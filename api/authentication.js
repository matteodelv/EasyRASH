         ...
         crypto.randomBytes(48, function(err, buffer) {
            ...
            fs.readFile('./storage/usersUnverified.json', 'utf8', (err, data) => {
               ...
               fs.writeFile('./storage/usersUnverified.json', JSON.stringify(usersUnverified, null, "\t"), err => {
                  if (err) throw err;
                  var verifyLink = req.get('host') ? req.protocol + '://' + req.get('host') + '/api/authentication/verify/' + token 
                  : req.protocol + '://' + server.address().address + ':' + server.address().port + '/api/authentication/verify/' + token;
